...
export default function SearchResult({ movies, query, loaderRef, hasMore, loading }:
{ movies: TMDBMovie[]; query: string; loaderRef: RefObject<HTMLDivElement | null>;
    ...
    return (
        <section className="bg-gray text-white flex flex-col gap-2">
            ...
            {loading ? <MovieSkeleton count={6} /> : movies.map(m => {
                const path = m.poster_path || m.backdrop_path || "";
                const src = path ? `${IMG_BASE}/w500${path}` : "/placeholder-portrait.png";
                return (
                    <Link
                        key={m.id} href={{ pathname: `/title/${m.id}`,
                        query: { img: path } }} prefetch={false}
                    ...
                    >
                        ...
                        <Image src="/icons/playSearch.svg" alt="" width={28} height={84} />
                    </Link>
                );
            })}
