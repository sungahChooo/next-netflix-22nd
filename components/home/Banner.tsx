...
export default function Banner() {
  ...
  const backdrop = useMemo(() => buildImageUrl(backdropPath, 'w1280'), [backdropPath]);
  const rank = Math.min(index + 1, 10);

  // img 쿼리로 경로전달
  const previewHref =
    current && backdropPath ? { pathname: `/title/${current.id}`, query: { img: backdropPath } } : '#';
    ...
    <section className="overflow-hidden bg-black text-white">
      {current && backdrop ? (
        <Link
          ...
          className="relative block h-[415px] cursor-pointer"
        >
          <Image
            key={backdrop}
            src={backdrop}
