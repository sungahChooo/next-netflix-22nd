...
export default function Banner() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await fetcher<tmdbListResponse<tmdbTitle>>('/3/movie/popular', {
          language: 'ko-KR',
          region: 'KR',
          page: '1',
        });
    ...
    <section className="rounded-xl overflow-hidden bg-black text-white">
      ...
      <div className="px-4 py-4">
        ...
        <div className="mx-auto flex w-[259px] h-[45px] justify-between">
          {/* My List */}
          <button className="flex flex-col w-[41px] h-[45px] items-center justify-center cursor-pointer">
            <Image src={add} alt="add icon" />
            <span className="text-xs">My List</span>
          </button>

          {/* Play */}
          <button className="flex items-center w-[110px] h-[45px] justify-center bg-white text-black rounded-md py-2 font-semibold cursor-pointer">
            <Image src={play} alt="play icon" />
            <span className="ml-[13px]">Play</span>
          </button>

          {/* Info */}
          <button className="flex flex-col w-[41px] h-[45px] items-center justify-center text-[13px] cursor-pointer">
